...
def logits_to_prob(logits: np.ndarray, num_classes: int = None, loss_func_name: str = None):
    """
    Convert logits to probabilities.
    Handles standard multiclass, CORAL cumulative, and CORN conditional logits.
    ...
    num_classes
        ...
        assumes an ordinal loss format (CORAL or CORN) and converts accordingly.
    loss_func_name
        The name of the loss function (e.g. "coral_loss", "corn_loss").
        When num_classes-1 outputs are detected, this disambiguates CORAL vs CORN.
        If None, defaults to CORAL-style conversion (backward compatible).
    ...
    if logits.ndim == 1:
        ...
        # Check if this is an ordinal loss format (num_classes - 1 logits)
        if num_classes is not None and logits.shape[1] == num_classes - 1:
            if loss_func_name is not None and "corn" in loss_func_name.lower():
                from ..optim.losses.corn_loss import corn_logits_to_probs
                return corn_logits_to_probs(logits, num_classes=num_classes, as_numpy=True)
            else:
                # Default: CORAL (also used when loss_func_name is None for backward compat)
