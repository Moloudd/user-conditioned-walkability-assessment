...
class MultiModalFeaturePreprocessor(TransformerMixin, BaseEstimator):
    ...
    def transform_prediction(
        ...
        loss_func_name
            ...
            Used to disambiguate ordinal loss prediction conversion when
            logits have num_classes-1 outputs.
        ...
        if self.label_type == CATEGORICAL:
            ...
            if len(y_pred.shape) == 2 and y_pred.shape[1] >= 2:
                ...
                if y_pred.shape[1] == num_classes - 1:
                    # Ordinal loss (CORAL or CORN): convert logits to class predictions
                    if loss_func_name is not None and "corn" in loss_func_name.lower():
                        from ..optim.losses.corn_loss import corn_logits_to_predictions
                        y_pred = corn_logits_to_predictions(y_pred, as_numpy=True)
                    else:
                        # Default: CORAL (also backward compat when loss_func_name is None)
