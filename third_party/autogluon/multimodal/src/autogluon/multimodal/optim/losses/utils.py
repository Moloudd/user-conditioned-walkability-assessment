    ...
    if problem_type in [BINARY, MULTICLASS]:
        if mixup_active:
            ...
            if loss_func_name is not None and loss_func_name.lower() == "focal_loss":
                ...
                importance_weights = kwargs.get("importance_weights", None)
                if importance_weights is None and config is not None:
                    if isinstance(config, (dict, DictConfig)):
                        importance_weights = config.get("importance_weights", None)
                    else:
                        importance_weights = getattr(config, "importance_weights", None)

                if importance_weights is not None:
                    if not isinstance(importance_weights, torch.Tensor):
                        if hasattr(importance_weights, '__iter__'):
                            importance_weights = [float(w) for w in importance_weights]
                        importance_weights = torch.tensor(importance_weights, dtype=torch.float32)
                    print(f"⚖️ Using Importance Weights in CORAL Loss: {importance_weights}")
            ...
            elif loss_func_name is not None and "corn" in loss_func_name.lower():
                if "num_classes" not in kwargs:
                    raise KeyError(
                        f"num_classes is missing from kwargs in get_loss_func. "
                ...
                if importance_weights is None and config is not None:
                    importance_weights = (
                        config.get("importance_weights", None)
                        if isinstance(config, (dict, DictConfig))
                        else getattr(config, "importance_weights", None)
                    )
                if importance_weights is not None and not isinstance(importance_weights, torch.Tensor):
                    importance_weights = torch.tensor(
                        [float(w) for w in importance_weights], dtype=torch.float32
                    )
                ...
                print(f"\n{'='*70}")
                print(f"🌽 CORN LOSS ACTIVATED!")
                print(f"   num_classes: {kwargs['num_classes']}")
                print(f"   output_dim: {kwargs['num_classes']-1} (conditional ordinal regression)")
                if importance_weights is not None:
                    print(f"   importance_weights: {importance_weights}")
                print(f"{'='*70}\n")
