...
class BaseLearner(ExportMixin, DistillationMixin, RealtimeMixin):
    ...
    def evaluate(
        ...
        **kwargs,
        ...
        metric_data = {}
        loss_func_name = getattr(getattr(self, '_config', None), 'optim', None)
        loss_func_name = getattr(loss_func_name, 'loss_func', None)
        if self._problem_type in [BINARY, MULTICLASS]:
            # Pass num_classes and loss_func_name to handle ordinal logits (num_classes-1 outputs)
            num_classes = self._output_shape if hasattr(self, '_output_shape') else None
            y_pred_prob = logits_to_prob(logits, num_classes=num_classes, loss_func_name=loss_func_name)
        ...
        **kwargs,
        ...
        if candidate_data:
            ...
            if self._df_preprocessor:
                _loss_func_name = getattr(getattr(self, '_config', None), 'optim', None)
                _loss_func_name = getattr(_loss_func_name, 'loss_func', None)
                pred = self._df_preprocessor.transform_prediction(
                    y_pred=logits,
                    loss_func_name=_loss_func_name,
                )
            else:
    ...
    def predict_proba(
        ...
        **kwargs,
        ...
        if candidate_data:
            ...
            _loss_func_name = getattr(getattr(self, '_config', None), 'optim', None)
            _loss_func_name = getattr(_loss_func_name, 'loss_func', None)
            prob = logits_to_prob(logits, num_classes=num_classes, loss_func_name=_loss_func_name)
