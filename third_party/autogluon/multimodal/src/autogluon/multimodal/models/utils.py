...
def get_model_postprocess_fn(problem_type: str, loss_func: _Loss):
    ...
    if problem_type == REGRESSION:
        ...
        from ..optim.losses.corn_loss import CornLoss, corn_logits_to_probs
        ...
        elif isinstance(loss_func, CornLoss):
            def apply_corn_postprocess(output: Dict, num_classes: int):
                for k, v in output.items():
                    if LOGITS in v:
                        output[k][LOGITS] = corn_logits_to_probs(v[LOGITS], num_classes, as_numpy=False)
                return output
            postprocess_func = functools.partial(apply_corn_postprocess, num_classes=loss_func.num_classes)
...
def create_fusion_model(
    ...
    try:
        ...
        coral_enabled = loss_name is not None and (
            "coral" in loss_name.lower() or "corn" in loss_name.lower()
        )
    except Exception:
        coral_enabled = False
